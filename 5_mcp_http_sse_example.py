...
class HTTPTransport:
    ...
    async def _sse_loop(self) -> None:
        if not self.session:
            return
        try:
            async with self.session.get(f"{self.config.base_url}/sse") as response:
...
async def main() -> None:
    ...
    config = HTTPTransportConfig(base_url="http://localhost:8000")

    # Start a local mock server for testing
    mock_runner = await start_mock_server(host="127.0.0.1", port=8000)
    transport = HTTPTransport(config)
    await transport.start()
    try:
        request = {"id": "test-1", "type": "request", "content": "Hello from client"}
        ...
        try:
            sse_msg = await asyncio.wait_for(transport.receive(), timeout=5.0)
